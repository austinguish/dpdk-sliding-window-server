...
std::unordered_map<uint64_t, double> packet_times;
...
static void send_packet(int flow_id, uint64_t seq_num)
{
    ...
    if (sent == 1)
    {
        ...
        // add the packet time
        // flow_table[flow_id].packet_times[seq_num] = {get_current_time(), 0};
        flow_table[flow_id].total_packets_sent++;
    ...
    while (active_flows > 0)
    {
        ...
        for (int i = 0; i < nb_rx; i++)
        {
            ...
            if (flow_id >= 0 && flow_id < flow_num)
            {
                rte_spinlock_lock(&flow_table[flow_id].lock);
                // set the acked time
                // flow_table[flow_id].packet_times[ack_num].acked_time = get_current_time();
                // update the acked flag
                // if ack_num%100 == 0 print the acked_time - send_time and convert to ms
                // if (ack_num % 100 == 0 || NUM_PING <= 100)
                // {
                //     uint64_t send_time = flow_table[flow_id].packets[ack_num].send_time;
                //     uint64_t acked_time = get_current_time();
                //     printf("Flow %d: Packet %lu RTT = %.3f ms\n", flow_id, ack_num,
                ...
                if (ack_num > flow_table[flow_id].last_acked)
                {
                    ...
                    // 清理已确认的包
                    for (auto it = flow_table[flow_id].packets.begin(); it != flow_table[flow_id].packets.end();)
                    {
                        if (it->first <= ack_num)
                        {
...
static int lcore_main(void* arg)
{
    int flow_id = *(int*)arg;
    delete (int*)arg;
    flow_table[flow_id].start_time = get_current_time();
    ...
    while (1)
    {
        ...
        while (flow_table[flow_id].next_seq_num <= flow_table[flow_id].last_acked + flow_table[flow_id].window_size &&
    ...
    flow_table[flow_id].end_time = get_current_time();
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < flow_num; i++)
    {
        ...
        flow_table[i].total_packets_sent = 0;
    }
    ...
    // calculate the throughput throughput = (double)total_data/1000/1000/1000 * 8 / (end_time - start_time);
    for (int i = 0; i < flow_num; i++)
    {
        printf("Flow %d: start_time = %lu, end_time = %lu\n", i, flow_table[i].start_time, flow_table[i].end_time);
        //
        // this time is in chrono::high_resolution_clock
        // convert to nanoseconds
        //using the thread start time and end time to calculate the total time

        // uint64_t total_time = std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time).count();
        // printf("Total time = %lu seconds\n", total_time);
         uint64_t total_data = flow_table[i].total_packets_sent * packet_len;
        uint64_t total_time = flow_table[i].end_time - flow_table[i].start_time;
        // convert to seconds in double
        double total_time_seconds = (double)total_time / 1e9;
        double throughput = (double)total_data / 1000 / 1000 / 1000 * 8 /total_time_seconds;
